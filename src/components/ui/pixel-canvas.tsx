...
class PixelCanvasElement extends HTMLElement {
  ...
  private readonly _boundAppear = () => this.handleAnimation("appear");
  private readonly _boundDisappear = () => this.handleAnimation("disappear");
  ...
  getDistanceToCenter(x: number, y: number) {
    const dx = x - this.logicalW / 2;
    const dy = y - this.logicalH / 2;
    return Math.sqrt(dx * dx + dy * dy);
  }

  getDistanceToBottomLeft(x: number, y: number) {
    const dx = x;
    const dy = this.logicalH - y;
    return Math.sqrt(dx * dx + dy * dy);
  }
  ...
  createPixels() {
    ...
    for (let x = 0; x < this.logicalW; x += this.gap) {
      for (let y = 0; y < this.logicalH; y += this.gap) {
        const color =
...
function ensureCustomElement() {
  if (typeof window === "undefined" || defined) return;
