...
export const Particles: React.FC<ParticlesProps> = ({
  ...
  initCanvasRef.current = () => { resizeCanvas(); clearContext(); circles.current = []; for (let i = 0; i < quantity; i++) drawCircle(circleParams()); };
  ...
  animateRef.current = () => {
    ...
    circles.current.forEach((circle, i) => {
      ...
      const rce = parseFloat(remapValue(closestEdge, 0, 20, 0, 1).toFixed(2));
      if (rce > 1) { circle.alpha += 0.02; if (circle.alpha > circle.targetAlpha) circle.alpha = circle.targetAlpha; }
      else circle.alpha = circle.targetAlpha * rce;
